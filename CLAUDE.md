...
- **商標・サービス名を使わない**: 外部サービスの固有名詞（Bill One等）は使わず、「請求書受領サービス」「勤怠管理システム」のように汎用的な表現にする
- **社内愛称・プロジェクト名を使わない**: 「タケヤくん」「プラサポ」等の社内向け名称は掲載しない。機能を表すタイトル（「備品補充ボット」「消耗品価格比較アプリ」等）にする
- **機能で表現する**: タイトル・説明文は「何ができるか」を端的に書く
